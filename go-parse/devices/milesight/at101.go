...
func decodeAT101(entries []TLV, deviceID, provider string, ts time.Time) []record.SensorDataRecord {
	...
	for _, e := range entries {
		...
		case (e.Channel == 0x04 || e.Channel == 0x84) && e.Type == 0x88:
			out = append(out, record.NewFloat(st.Latitude, dm, deviceID, provider, float64(int32(le32(e.Data[0:4])))/1000000.0, ts))
			out = append(out, record.NewFloat(st.Longitude, dm, deviceID, provider, float64(int32(le32(e.Data[4:8])))/1000000.0, ts))
			status := e.Data[8]
