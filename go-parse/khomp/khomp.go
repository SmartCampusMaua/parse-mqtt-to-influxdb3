// Package khomp decodes Khomp IoT LoRaWAN sensor payloads.
// Implements the Khomp IoT binary protocol for fPort 4 (NIT 21LI)
// as documented in khomp.js.
package khomp
...
// KhompSensors holds all decoded sensor fields from a Khomp NIT 21LI payload.
// Field names match the Khomp IoT protocol register map (fPort 4).
type KhompSensors struct {
...
// parseKhompSensors implements the Khomp fPort-4 binary protocol (khomp.js, NIT 21LI).
// Returns a *KhompSensors with all decoded fields populated.
func parseKhompSensors(bytes []byte) (*KhompSensors, error) {
	p := &KhompSensors{}
	index := 0
	...
	// Extension module EMW104 (module ID = 0x04)
...
// jsonKhompSensors wraps KhompSensors for JSON serialisation (legacy compat).
func jsonKhompSensors(ks *KhompSensors) string {
